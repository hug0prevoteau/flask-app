...
def hello_world(source=None, destination=None):
    passdata = PassportData()
    result = {'source': source, 'destination': destination, 'treatment': passdata.get_visa_data(source, destination)}
...
class PassportData:
    ...
    def get_visa_data(self, source, destination):
        """Downloads visa status CSV data for all Countries"""
        self.get_country_codes_iso2()
        self.get_passport_data()
        self.clean_passport_data()
        visa_data_raw = self.get_passport_data_country_names_matrix()
        return visa_data_raw.loc[source][destination]
